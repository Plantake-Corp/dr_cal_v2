import { create, insertMany, deleteOne, findByIdAndUpdate, find, aggregate } from '../models/food';
...
const inserts = asyncHandler(async (req, res) => {
    ...
    try {
        const response = await insertMany(body);
        if (!response) { 
...
const deleteById = asyncHandler(async (req, res) => {
    ...
    try {
        const response = await deleteOne({_id: id});
...
const updateById = asyncHandler(async (req, res) => {
    ...
    try {
        const response = await findByIdAndUpdate(fields, updates, options);
...
const getById = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const response = await find({_id: id});
        if (!response){
            return res.status(404).json({error: "Not Found!"});
        }
        res.status(200).json({data: response});
...
const getByQuery = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (search){
            ...
            results = await find().limit(limit);
        }
